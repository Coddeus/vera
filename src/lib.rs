...
use vulkano::memory::allocator::{AllocationCreateInfo, MemoryUsage, StandardMemoryAllocator, GenericMemoryAllocator};
...
pub struct Vera {
    // Vulkan stuff (yeah it's built ***on top of*** Vulkan)
    ...
    device: Arc<Swapchain>,
    // queues: impl ExactSizeIterator,
    queue: Arc<Queue>,
    ...
    // fences: Vec<Option<Arc<FenceSignalFuture<impl GpuFuture>>>>,
    previous_fence_i: u32,

    // Vera stuff (The few things I actually added)
...
impl Vera {
    ...
    pub fn run(vertices: Vec<MyVertex>) {
        ...
        event_loop.run(move |event, _, control_flow| match event {
            ...
            Event::MainEventsCleared => {
                if window_resized || recreate_swapchain {
                    ...
                    let new_framebuffers = Self::get_framebuffers(&new_images, render_pass.clone());
                    ...
                    if window_resized {
                        ...
                        command_buffers = Self::get_command_buffers(
                            &command_buffer_allocator,
                            &queue,
                            &new_pipeline,
                            &new_framebuffers,
